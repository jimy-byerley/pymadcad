...
def intersect_triangles(f0, f1, double precision):
	...
	cdef double sA
	cdef size_t iA1, iA2
	sA = max(max_dv(vabs(xA), 3, &iA1), max_dv(vabs(yA), 3, &iA2))
	
	#sB=max(max_v(abs(xB))[0], max_v(abs(yB))[0])
	cdef double sB
	cdef size_t iB1, iB2
	sB = max(max_dv(vabs(xB), 3, &iB1), max_dv(vabs(yB), 3, &iB2))
	
	# identify signs of yA and yB
	cdef int sYA[3]
	cdef int sYB[3]
	for i in range(3):
		if fabs(varr(&yA)[i])/sA < precision:
			sYA[i]=0
			varr(&yA)[i]=0.
		else : 
			sYA[i]=sign(varr(&yA)[i])
		if fabs(varr(&yB)[i])/sB < precision:
			sYB[i]=0
			varr(&yB)[i]=0.
		else : 
			sYB[i]=sign(varr(&yB)[i])

	# check if triangles have no intersections with line D
	if fabs(sum_iv(sYA, 3))==3 or fabs(sum_iv(sYB,3))==3 : 
	...
	cdef int neIB=0
	for i in range(3):
		if sYA[i]*sYA[(i+1)%3]<0 and fabs(sYA[i])+fabs(sYA[(i+1)%3])>0 : 
			eIA[neIA]=i
			neIA=neIA+1
		elif sYA[i]==0 :
			if sYA[(i+1)%3] != 0 : 
				...
				eIA[neIA]=(i-1)%3
				neIA=neIA+1
		if sYB[i]*sYB[(i+1)%3]<0 and fabs(sYB[i])+fabs(sYB[(i+1)%3])>0 : 
			eIB[neIB]=i
			neIB=neIB+1
		elif sYB[i]==0 :
			if sYB[(i+1)%3] != 0 : 
...
# main intersection routine
def intersect_triangles(f0, f1, precision=1e-7):
	''' Intersects 2 triangles and outputs intersections vertices
