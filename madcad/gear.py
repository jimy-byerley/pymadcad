...
def repeat_circular(gear_profile: Wire, z: int) -> Web:
	...
	Parameters:
		...
		gear_profile (Wire): Wire (profile of the pinion generated by the function `gearprofile`)
		z (int): number of teeth
	"""
	# Parameters
	angle = anglebt(gear_profile[0], gear_profile[-1])
...
def create_pattern_rect(
	...
	**kwargs,
	...
	if rounded:  # rounded case
		...
		bevel(pattern_ref, groups, ("radius", 0.3 * (r_ext - r_int)))
		pattern_ref.mergeclose()
	pattern_profile = repeat(pattern_ref, patterns, rotatearound(angle_step, axis))
	...
	**kwargs,
	...
	Parameters:
		...
		ext_radius (float):
				...
				the external radius of the structure part
				by taking the smallest radius of the profile of `gearprofile`, `ext_radius` must be smaller than it
