...
for w in cases():
	...
	try:
		...
		show([w, [note_floating(p, text=' '+str(i)) for i,p in enumerate(w.points)]], display_points=True)
		raise
	try:
		f.check()
		assert f.issurface()
		# show([w, f], display_points=True, display_wire=True)
		show([w, f, [note_floating(p, text=' '+str(i)) for i,p in enumerate(f.points)]], display_points=True, display_wire=True)
	except:
		print(w.edges)
